climate_data_ncep_aggregate<-function(year){
  ...
  air.temp.file<-nc_open(paste('/data/ifs/users/yzhang/DATA/NCEP-DOE/air.2m.gauss.',year,'.nc',sep=''))
  radiation.file<-nc_open(paste('/data/ifs/users/yzhang/DATA/NCEP-DOE/dswrf.sfc.gauss.',year,'.nc',sep=''))
  outfile<-c(paste('/data/ifs/users/yzhang/DATA/NCEP-DOE/processed/air.2m.8day.',year,'.nc',sep=''),
             paste('/data/ifs/users/yzhang/DATA/NCEP-DOE/processed/rad.8day.',year,'.nc',sep=''))
  ...
  time.output<-air.temp.file$var[[1]]$dim[[4]]$vals[(0:45)*32+1]
  ...
  time.output<-radiation.file$var[[1]]$dim[[2]]$vals[(0:45)*8+1]
  
  x<-ncdim_def('Lon','degreesE',radiation.file$var[[2]]$dim[[1]]$vals)
  y<-ncdim_def('Lat','degreesN',radiation.file$var[[2]]$dim[[2]]$vals)
  t<-ncdim_def("Time",'hours since 1800-01-01 00:00:0.0',time.output,unlim=TRUE)
